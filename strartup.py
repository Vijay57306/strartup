...
# 🔧 Column name fixes (to match your dataset)
...
def render_line_chart(data, x, y, title):
    fig, ax = plt.subplots(figsize=(6, 4))
    sns.lineplot(data=data, x=x, y=y, marker="o", ax=ax, color="blue")
    ax.set_title(title)
    ax.tick_params(axis="x", rotation=45)
    st.pyplot(fig)
...
# --- Data Preview ---
st.subheader("📊 Filtered Dataset (All Records)")
...
if filtered_df.empty:
    ...
    with col5:
        ...
        industry_count = filtered_df["Industry Vertical"].value_counts().reset_index()
    ...
    with col6:
        ...
        city_funding = (
            filtered_df.groupby("City Location")["Amount in USD"]
            .sum()
            .sort_values(ascending=False)
        )
        if show_top_10:
    ...
    with col8:
        ...
        city_count = filtered_df["City Location"].value_counts().reset_index()
